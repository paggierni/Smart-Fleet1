...
char* HTTPPost;
const char* HTTPheader1 = "POST / HTTP/1.1\r\n";
char* HTTPheader2;
const char* HTTPheader3 = "Accept: application/json\r\n";
const char* incompleteHTTPheader4 = "Content-Length: ";
const char* HTTPheader5 = "Content-Type: application/json\r\n\r\n{\r\n";
const char* host = "Host: ";
const char* newLine = "\r\n";
const char* wordData = "\"Data\": \"";
const char* closeJSON = "\r\n}\r\n";
const char* closeQote = "\"";
...
bool transmit()
{
  ...
  //Contruccion del HTTP POST
  //if (HTTPPost != NULL){
  //  free(HTTPPost);}
  char* HTTPheader4; 
  HTTPheader4 = (char*) malloc(strlen(incompleteHTTPheader4)+strlen(cache.buffer())+strlen(newLine));
  //strcpy(HTTPheader4, wordData);
  strcat(HTTPheader4, incompleteHTTPheader4);

  //char* buffer_len = (char*) strlen(cache.buffer());
  unsigned int len = strlen(wordData) + cache.length()+strlen(closeQote);
  char stelen[4];
  sprintf(stelen, "%d", len);
  strcat(HTTPheader4, stelen);
  strcat(HTTPheader4, newLine);
  //Serial.println(HTTPheader4);
  //char* transmitData;
  //transmitData = (char*) malloc(strlen(cache.buffer())+strlen(wordData)+strlen(closeQote));
  //strcat(transmitData, wordData);
  //strcat(transmitData, cache.buffer());
  //strcat(transmitData, closeQote);

  //Se arma todo 

  HTTPPost = (char*) malloc(strlen(HTTPheader1)+ strlen(HTTPheader2)+strlen(HTTPheader3)+strlen(HTTPheader4)+strlen(HTTPheader5)+strlen(wordData)+strlen(cache.buffer())+strlen(closeQote)+strlen(closeJSON));

  strcpy(HTTPPost, HTTPheader1);
  strcat(HTTPPost, HTTPheader2);
  strcat(HTTPPost, HTTPheader3);
  strcat(HTTPPost, HTTPheader4);
  strcat(HTTPPost, HTTPheader5);
  strcat(HTTPPost, wordData);
  strcat(HTTPPost, cache.buffer());
  strcat(HTTPPost, closeQote);
  strcat(HTTPPost, closeJSON);

  Serial.println(HTTPPost);
  unsigned int HTTPPostLength = (int) strlen(HTTPPost);
  Serial.println(HTTPPostLength);
  ...
  // transmit data
  if (net.send(HTTPPost,HTTPPostLength)) {
    connErrors = 0;
    txCount++;
    // output some stats
    Serial.print('#');
